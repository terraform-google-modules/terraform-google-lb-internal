# Internal HTTP(S) Load Balancer Terraform Module
Modular Internal HTTP(S) Load Balancer for GCE using forwarding rules.
...
```hcl
module "gce-http-ilb" {
  source       = "GoogleCloudPlatform/lb-internal/google//modules/l7-lb"
  version      = "~> 2.0"
  region       = var.region
  name         = "group2-ilb"
  ports        = ["80"]
