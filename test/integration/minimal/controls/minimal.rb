...
gcloud_args = "--format json --project #{project_name}"
system("gcloud auth activate-service-account --key-file #{ENV['GOOGLE_APPLICATION_CREDENTIALS']}")

control 'lb-internal-minimal' do
  title 'A minimal example of running an internal load balancer.'

  describe google_compute_firewall(project: project_name, name: "#{resource_name_prefix}-ilb-fw") do
  ...
  describe google_compute_firewall(project: project_name, name: "#{resource_name_prefix}-hc") do
  ...
  describe google_compute_forwarding_rule(project: project_name, region: region, name: "#{resource_name_prefix}") do
  ...
  describe 'The health check' do
    ...
    it 'is listening on 8080' do
      expect(health_checks[0]['tcpHealthCheck']['port']).to eq(8081)
    end
    it 'is of type TCP' do
      expect(health_checks[0]['type']).to eq('TCP')
    end
  end
